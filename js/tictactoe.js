...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    x2 = coordX2,
     ...
     y2 = coordY2,
     //This var stores temporary x axis data we updade in our animation loop.
     x = x1,
     //This var stores temporary y axis data we updade in our animation loop.
     y = y1;
...
function animateLineDrawing() {
    ...
    if (x1 <= x2 && y1 <= y2) {
        //This condition adds to 10 to the previous end x point.
        if ( x < x2) { x+= 10;}
        //This condition adds to 10 to the previous end y point.
        if ( y < y2) { y+= 10;}
        //This condition cancels our animation loop
        //if we have reached the end poits.
        if(x >= x2 && y >= y2) {cancelAnimationFrame(animationLoop);}
    }
    //This condition is similar to the one above
    //This is required for the 6, 4, 2 win con.
    if (x1 <= x2 && y1 >= y2) {
        if ( x < x2) { x += 10;}
        if ( y > y2) { y -= 10;}
        if(x >= x2 && y <= y2) {cancelAnimationFrame(animationLoop);}
...
function resetGame() {
    ...
    //This resets our array so it is emptyand we can start over.
    selectedSquares = [];
}